...
const SalesTable: React.FC<SalesTableProps> = ({ records, selectedWeek, onRecordUpdate }) => {
  ...
  const calculateSales = (record: SalesRecord) => {
    const workDays = record.schedule.filter(Boolean).length;
  ...
  const handleHostStoreAdd = (recordId: number, store: string) => {
    onRecordUpdate(recordId, { hostStore: store });
  };
  ...
  return (
    <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto', maxHeight: '80vh' }}>
      ...
      {/* 代理店 */}
        ...
        >
                  {hasWork && <CheckIcon color="primary" fontSize="small" />}
        </TableCell>
      ))}
              ...
              <TableCell sx={{ 
                width: '80px',
                backgroundColor: record.dayType === '平日' ? '#f5f5f5' : '#fff9c4',
                cursor: editingRecord === record.id ? 'pointer' : 'default',
                '&:hover': editingRecord === record.id ? { opacity: 0.8 } : {}
              }}
              onClick={(e) => handleDayTypeClick(e, record.id)}
              >
        <Typography variant="body2" sx={{ whiteSpace: 'nowrap' }}>{record.dayType}</Typography>
      </TableCell>
          ...
          <Select
            ...
            size="small"
                ...
                sx={{
                              ...
                              >
                                {availableStores
                                  .filter(store => !record.partnerStores.includes(store) && store !== record.hostStore)
                                  .map((store) => (
                                    <MenuItem key={store} value={store}>
                                      {store}
                                    </MenuItem>
      ...
      {locationReservationModal && (
        ...
        >
          <DialogTitle sx={{ 
            ...
            <IconButton onClick={handleLocationReservationModalClose} size="small">
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          ...
          <DialogContent sx={{ p: 3 }}>
            {(() => {
              const record = records.find(r => r.id === locationReservationModal.recordId);
              if (!record) return null;
              
              const reservations = record.locationReservations || [];
                ...
                <Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Button
                      variant="contained"
                      size="small"
                      onClick={() => handleLocationReservationAdd(locationReservationModal.recordId)}
                      sx={{ minWidth: '100px' }}
                    >
                      + 追加
                    </Button>
                    ...
                    <Table size="small" sx={{ border: '1px solid #e0e0e0' }}>
                      <TableHead>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ fontWeight: 'bold', width: '100px' }}>ステータス</TableCell>
                          <TableCell sx={{ fontWeight: 'bold', width: '150px' }}>手配会社</TableCell>
                          <TableCell sx={{ fontWeight: 'bold', width: '100px', textAlign: 'right' }}>卸単価</TableCell>
                          <TableCell sx={{ fontWeight: 'bold', width: '100px', textAlign: 'right' }}>仕入単価</TableCell>
                          <TableCell sx={{ fontWeight: 'bold', width: '60px', textAlign: 'center' }}>操作</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
                          ...
                          <TableRow key={reservation.id}>
                            <TableCell sx={{ width: '120px' }}>
                              <TextField
                                ...
                                onChange={(e) => handleLocationReservationUpdate(
                                  locationReservationModal.recordId, 
                                  reservation.id, 
                                  { date: e.target.value }
                                )}
                                size="small"
                            ...
                            <TableCell sx={{ width: '100px' }}>
                              <FormControl size="small" sx={{ width: '90px' }}>
                                ...
                                >
                                  ...
                                  <MenuItem value="お断り">お断り</MenuItem>
                                </Select>
                              </FormControl>
                            </TableCell>
                            <TableCell sx={{ width: '150px' }}>
                              <TextField
                                ...
                                onChange={(e) => handleLocationReservationUpdate(
                                  locationReservationModal.recordId, 
                                  reservation.id, 
                                  { arrangementCompany: e.target.value }
                                )}
                                size="small"
                                variant="outlined"
                                placeholder="手配会社名"
                ...
                sx={{ 
                            ...
                            <TableCell sx={{ width: '60px', textAlign: 'center' }}>
                              <IconButton
                                size="small"
                                color="error"
                                onClick={() => handleLocationReservationRemove(
                                  locationReservationModal.recordId, 
                                  reservation.id
                                )}
                              >
          ...
          <DialogActions sx={{ p: 2, borderTop: '1px solid #e0e0e0' }}>
            <Button onClick={handleLocationReservationModalClose} variant="outlined">
              閉じる
            </Button>
          </DialogActions>
        </Dialog>
      ...
      >
        <Box sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (detailDrawer?.recordId) {
                          onRecordUpdate(detailDrawer.recordId, { detailStatus: '公開済み' });
                        }
                      }}
              sx={{ 
                        minWidth: '80px',
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (detailDrawer?.recordId) {
                          onRecordUpdate(detailDrawer.recordId, { detailStatus: '未登録' });
                        }
                      }}
                      sx={{
                        minWidth: '80px',
            ...
            <IconButton onClick={handleDetailDrawerClose} size="small">
              <CloseIcon />
            </IconButton>
            </Box>
          </Box>
                ...
                <Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 5, mt: 3 }}>
                    ...
                    <Box sx={{ flex: '0 0 320px', display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                      {/* 代理店名 */}
                      <Box sx={{ 
                        backgroundColor: '#e8d5f0', // 濃い紫から薄い紫に変更
                        color: '#6a1b9a', // テキスト色を濃い紫に変更
                        px: 2,
                        py: 0.8,
                        borderRadius: '8px',
                        display: 'inline-block',
                        alignSelf: 'flex-start',
                        boxShadow: '0 2px 4px rgba(232, 213, 240, 0.3)' // シャドウも薄く調整
                      }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Table sx={{ 
                        ...
                        <TableHead>
                          <TableRow sx={{ backgroundColor: '#f5f6fa' }}> {/* 濃いグレーから薄いグレーに変更 */}
                            <TableCell sx={{ 
                              fontWeight: 'bold', 
                              border: '1px solid #e1e5e9', // ボーダーも薄く調整
                              padding: '12px 16px', 
                              fontSize: '0.9rem', 
                              whiteSpace: 'nowrap',
                              color: '#2c3e50' // 白から濃いグレーに変更
                            }}>役割</TableCell>
                            <TableCell sx={{ 
                              fontWeight: 'bold', 
                              border: '1px solid #e1e5e9', // ボーダーも薄く調整
                              ...
                              return (
                                <TableCell 
                                  ...
                                  sx={{ 
                                    ...
                                    minWidth: '90px',
                                    color: '#2c3e50', // 白から濃いグレーに変更
                                    backgroundColor: isWeekend ? '#fef7f0' : '#f5f6fa' // 週末も薄い色に調整
                                  }}
                                >
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <Typography variant="caption" sx={{ 
                                    ...
                                    {record.freeEntry[`day${workDate.index + 1}`] || 0}
                                  </Typography>
                                </TableCell>
                              );
                            })}
                  ...
                  <Box sx={{ mt: 4 }}>
                    <Accordion 
                      expanded={detailInfoExpanded} 
                      onChange={() => setDetailInfoExpanded(!detailInfoExpanded)}
                      sx={{ 
                        boxShadow: 'none',
                        border: '1px solid #e0e0e0',
                        borderRadius: '8px',
                    ...
                    >
                      ...
                      >
                        <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#2c3e50', fontSize: '1rem' }}>
                          詳細情報
                        </Typography>
                      </AccordionSummary>
                            ...
                            {detailEditMode ? (
                              <Box sx={{ display: 'flex', gap: 1, flex: 1 }}>
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { fieldContactName: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                                ...
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { fieldContactPhone: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            {detailEditMode ? (
                              <TextField
                                ...
                                onChange={(e) => {
                                  if (detailDrawer?.recordId) {
                                    onRecordUpdate(detailDrawer.recordId, { otherCompany: e.target.value });
                                  }
                                }}
                                size="small"
                                variant="outlined"
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            {detailEditMode ? (
                              <TextField
                                ...
                                onChange={(e) => {
                                  if (detailDrawer?.recordId) {
                                    onRecordUpdate(detailDrawer.recordId, { regularStaff: e.target.value });
                                  }
                                }}
                                size="small"
                                variant="outlined"
                            ...
                            {detailEditMode ? (
                              <Box sx={{ display: 'flex', gap: 1, flex: 1 }}>
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { meetingTime: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                                ...
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { meetingPlace: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                            ...
                            {detailEditMode ? (
                              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flex: 1 }}>
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { workStartTime: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                                ...
                                <TextField
                                  ...
                                  onChange={(e) => {
                                    if (detailDrawer?.recordId) {
                                      onRecordUpdate(detailDrawer.recordId, { workEndTime: e.target.value });
                                    }
                                  }}
                                  size="small"
                                  variant="outlined"
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            {detailEditMode ? (
                              <TextField
                                ...
                                onChange={(e) => {
                                  if (detailDrawer?.recordId) {
                                    onRecordUpdate(detailDrawer.recordId, { uniform: e.target.value });
                                  }
                                }}
                                size="small"
                                variant="outlined"
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            {detailEditMode ? (
                              <TextField
                                ...
                                onChange={(e) => {
                                  if (detailDrawer?.recordId) {
                                    onRecordUpdate(detailDrawer.recordId, { target: e.target.value });
                                  }
                                }}
                                size="small"
                                variant="outlined"
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                            ...
                            {detailEditMode ? (
                              <TextField
                                ...
                                onChange={(e) => {
                                  if (detailDrawer?.recordId) {
                                    onRecordUpdate(detailDrawer.recordId, { specialNotes: e.target.value });
                                  }
                                }}
                                size="small"
                                variant="outlined"
                              ...
                              <Typography variant="body2" sx={{ flex: 1, color: record?.specialNotes ? '#2c3e50' : '#999', whiteSpace: 'pre-line' }}>
                  ...
                  <Box sx={{ mt: 3 }}>
                    ...
                    <Box sx={{ mb: 3, maxHeight: '400px', overflowY: 'auto' }}>
                      {record?.communications && renderAllCommunications(record.communications)}
