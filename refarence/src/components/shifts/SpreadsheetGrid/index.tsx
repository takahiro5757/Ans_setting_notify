      ...
      <Table>
        ...
        <TableBody>
          ...
          {dates.map(({ date, dayOfWeek, isWeekend }) => {
            ...
            let cellClassName = '';
            if (isSunday) {
              cellClassName = 'weekend-sun';
            } else if (isSaturday) {
              cellClassName = 'weekend-sat';
            }
                ...
                {staffMembers.map(staff => {
                  ...
                  return (
                    <React.Fragment key={`${date.toISOString()}-${staff.id}`}>
                      <StyledTableCell className={`status-column ${cellClassName} ${shift?.status === '○' ? 'shift-available' : ''}`} sx={{ color: '#000000', fontWeight: shift?.status === '○' ? 700 : 600 }}>
                        {shift?.status || '-'}
                      </StyledTableCell>
