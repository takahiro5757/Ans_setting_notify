...
export const SubcontractorManagement: React.FC = () => {
  ...
  const handleEditSubcontractor = (subcontractor: SubcontractorData) => {
    ...
    setSubcontractorData({
      ...
      adminPassword: subcontractor.adminPassword,
  ...
  const handleSystemIssue = async (subcontractorId: string) => {
    ...
    setSubcontractorList(prev => prev.map(s => 
      s.id === subcontractorId 
        ? { ...s, systemStatus: 'active', systemUrl, initialPassword }
        : s
    ));
  ...
  const handleShowSystemInfo = (subcontractorId: string) => {
    ...
    setSystemInfoDialog({
      ...
      initialPassword: subcontractor.initialPassword || '※パスワードが記録されていません',
      isNewIssue: false
    });
  };
          ...
          <Grid container spacing={3} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                label="パスワード"
                type="password"
                value={subcontractorData.adminPassword}
                onChange={(e) => setSubcontractorData({...subcontractorData, adminPassword: e.target.value})}
                required
                fullWidth
          ...
          <Paper sx={{ p: 3, bgcolor: 'grey.50' }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  label="初期パスワード"
                  value={systemInfoDialog.initialPassword}
                  fullWidth
                  InputProps={{
                    readOnly: true,
                    endAdornment: (
                      <IconButton
                        onClick={() => handleCopy(systemInfoDialog.initialPassword, '初期パスワード')}
                        size="small"
                      >
                        <ContentCopyIcon />
                      </IconButton>
