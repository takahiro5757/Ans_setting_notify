...
export default function FreeEstimatePage() {
  ...
  const [items, setItems] = useState<FreeEstimateItem[]>([
    {
      id: 1,
      eventDate: '',
      itemName: '',
  ...
  // 品目更新
  const handleItemChange = (id: number, field: keyof FreeEstimateItem, value: any) => {
  ...
  // 金額計算
  const calculateAmount = (item: FreeEstimateItem) => {
  ...
  const handleCreateEstimate = () => {
    if (!agencyName || mainStores.length === 0) {
      alert('必須項目を入力してください。');
      return;
    }

    const { total } = calculateTotal();
    const generatedFileName = `見積書_${agencyName}_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    router.push('/accounting/estimates?from=custom');
  };
      ...
      <Container maxWidth={false} sx={{ position: 'relative' }}>
        ...
        <Paper sx={{ p: 3, mb: 3 }}>
          <Box sx={{ maxWidth: 900, mx: 'auto' }}>
            {/* ヘッダー：戻るボタン・タイトル・見積作成ボタン */}
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <IconButton 
                  onClick={() => router.push('/accounting/estimates')} 
                  sx={{ mr: 2, p: 0.5 }}
                >
                  <ArrowBackIcon />
                </IconButton>
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  onChange={(e) => setEstimateDeadline(e.target.value)}
                  InputLabelProps={{
                    shrink: true,
                  }}
                />
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 'medium' }}>見積品目</Typography>
              <Button
                variant="outlined"
                startIcon={<AddIcon />}
